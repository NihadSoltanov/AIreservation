...
import { BookOpen, Plus, Search, Trash2, Upload, FileText, HelpCircle } from "lucide-react"
...
import { MOCK_KNOWLEDGE, MOCK_ORG } from "@/lib/mock-data"
...
export default function KnowledgePage() {
  ...
  const faqs = MOCK_KNOWLEDGE.filter(k => k.type === "faq" &&
    ...
    <div className="space-y-6">
            ...
            <Card className="border-dashed" style={{ borderColor: ac + "50" }}>
              <CardContent className="p-5 space-y-3">
                ...
                <div className="flex gap-2">
                  <Button size="sm" style={{ background: ac }}>Save FAQ</Button>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-xs text-[var(--muted)]">
                            Added {formatRelativeTime(faq.created_at)}
                          </span>
                          <Badge variant="secondary" className="text-xs">FAQ</Badge>
                        </div>
                      </div>
                    </div>
                    <button className="text-[var(--muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1">
      ...
      {tab === "docs" && (
        <div className="space-y-4">
          ...
          {/* Existing docs */}
          <div className="space-y-3">
            {MOCK_KNOWLEDGE.filter(k => k.type === "document").map(doc => (
              <Card key={doc.id} className="group">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <button className="text-[var(--muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                </CardContent>
